...
// Коды команд для esp_now_cmd_t.command
#define CMD_REQ_STATUS           0 // Запрос статуса
#define CMD_START_DOSING         1 // Старт дозирования (volume - целевой объем)
#define CMD_STOP_DOSING          2 // Стоп дозирования
#define CMD_SET_TEMP_SETPOINT    3 // Обновить уставку температуры (temp - новая уставка)
#define CMD_SET_MOTOR_SPEED      4 // Обновить скорость мотора (speed - новая скорость)
#define CMD_START_MOTOR_CAL      5 // Старт калибровки мотора (actualVolume - подсказка целевого объема)
#define CMD_STOP_MOTOR_CAL       6 // Стоп калибровки мотора (actualVolume - фактический объем)
#define CMD_EMERGENCY_STOP      10 // Экстренная остановка
#define CMD_CLEAR_ERROR         11 // Сброс текущей системной ошибки

// Команда, получаемая от дисплея (СИНХРОНИЗИРОВАННАЯ СТРУКТURA)
