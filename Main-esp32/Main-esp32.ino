...
static void log_printf(const char* level, const char* tag, const char* format, va_list args) { // Made static as it's used by log_x in this file only now
    ...
    if (g_littlefs_mounted) {
        ...
        if (logFile) {
            ...
            static bool fs_log_fail_logged_once = false;
            if (!fs_log_fail_logged_once) {
...
int btn_fwd_state = HIGH, btn_rev_state = HIGH, btn_reset_state = HIGH, btn_power_state = HIGH;
...
int last_btn_fwd_state = HIGH, last_btn_rev_state = HIGH, last_btn_reset_state = HIGH, last_btn_power_state = HIGH;
unsigned long last_btn_fwd_change = 0, last_btn_rev_change = 0, last_btn_reset_change = 0, last_btn_power_change = 0;
...
// unsigned long systemStartTime = 0; // Moved to utils.c
    ...
    if (c_ms - last_btn_fwd_change > debounce_delay_ms) {
        if (fwd != btn_fwd_state) {
            ...
            if (btn_fwd_state == LOW) {
                if (system_power_enabled) manualMotorForward(); // Вызов из motor_control.h
            } else {
                if (motor_running_manual && motor_dir == HIGH) stopManualMotor(); // Вызов из motor_control.h
            }
        }
    }
    ...
    if (c_ms - last_btn_rev_change > debounce_delay_ms) {
        if (rev != btn_rev_state) {
            ...
            if (btn_rev_state == LOW) {
                if (system_power_enabled) manualMotorReverse(); // Вызов из motor_control.h
            } else {
                if (motor_running_manual && motor_dir == LOW) stopManualMotor(); // Вызов из motor_control.h
            }
        }
    }
